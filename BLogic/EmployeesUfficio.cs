...
{
    internal class EmployeesUfficio
    {
        public static List<Employees> PrintEmployeesUfficio()
        {
            List<Employees> listEmployeesUfficio = [];
            List<EmployeesActivities> listEmployeesActivitiesUfficio = [];
            ...
            foreach (string line in employeeTxt)
            {
                ...
                listEmployeesUfficio.Add(new Employees
                {
                    Matricola = fields[0],
            ...
            foreach (string line in employeeActivitiesTxt)
            {
                ...
                listEmployeesActivitiesUfficio.Add(new EmployeesActivities
                {
                    Data = fields[0],
                    Luogo = fields[1],
            ...
            Console.WriteLine("Lavoratori in ufficio di Betacom Group");
            Console.WriteLine(rowSeparator);
            Console.ForegroundColor = ConsoleColor.White;

            foreach (var employeeActivity in listEmployeesActivitiesUfficio)
            {
                if (employeeActivity.Luogo == "Ufficio")
                {
                    foreach (var employee in listEmployeesUfficio)
                    {
                        if (employeeActivity.Matricola == employee.Matricola)
                        {
                            Console.WriteLine($"Il dipendente {employee.NomeCognome} n° {employee.Matricola} è stato in ufficio in data {employeeActivity.Data}");
                        }
                        else
                        {
                            continue;
